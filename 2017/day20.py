...
data = """p=<1500,413,-535>, v=<-119,22,36>, a=<-5,-12,3>
...
p=<-2954,-985,167>, v=<-418,-140,23>, a=<29,7,-3>""".split('\n')

particles = list()
for line in data:
    particles.append(line.replace('p=<', '').replace('v=<', '').
                     replace('a=<', '').replace('>', '').split(','))
...
for nr, item in enumerate(particles):
    for pos, char in enumerate(item):
        particles[nr][pos] = int(particles[nr][pos])

for nr, item in enumerate(particles):
    vectorPosition.append(np.array(item[:3]))
    vectorVelocity.append(np.array(item[3:6]))
    vectorAccel.append(np.array(item[6:9]))
    vectorPosAbs.append(0)

for i in range(5000):
    for nr, velocity in enumerate(vectorVelocity):
        vectorVelocity[nr] += vectorAccel[nr]
        vectorPosition[nr] += vectorVelocity[nr]
        vectorPosAbs[nr] = sum(abs(vectorVelocity[nr]))

    # check if particles collide
    equalPos = list()
    for nrAct in range(len(vectorPosition)):

        for nrCheck in range(len(vectorPosition)):
            if nrAct != nrCheck:
                equal = 0
                for coord in range(3):
                    if vectorPosition[nrAct][coord] == vectorPosition[nrCheck][coord]:
                        equal += 1
                if equal == 3:
                    if equalPos.count(nrAct) == 0:
                        equalPos.append(nrAct)
                    if equalPos.count(nrCheck) == 0:
                        equalPos.append(nrCheck)
                    #print('turn:', i, 'qual:', nrAct, nrCheck)
    if len(equalPos) != 0:
        print(equalPos)
        for index in equalPos[::-1]:
            vectorAccel.pop(index)
            vectorPosition.pop(index)
            vectorVelocity.pop(index)
            vectorPosAbs.pop(index)
    print('len:', len(vectorPosition))

print(min(vectorPosAbs))
print(vectorPosAbs.index(min(vectorPosAbs)))
# for nr, pos in enumerate(vectorPosition):
#     print('nr:', nr, sum(abs(pos)))
