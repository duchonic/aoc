...
void showCave(std::vector< std::vector<int>> cave) {
	for (auto endl : range(1,50)) {
		std::cout << std::endl;
	}
	for (auto y : cave) {
		for (auto x : y) {
...
int maxCave(std::vector< std::vector<int>> cave) {
	...
	return sizes.at(249) * sizes.at(248) * sizes.at(247);
}
...
int main() {
	...
	std::cout << "part2 : " << maxCave(cave) << std::endl;
