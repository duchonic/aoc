...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan  6 09:44:23 2019

DAy 8 aoc2017 part 1
...
buw inc 559 if okl <= 1919""".split('\n')


def runReg(register, commands):
    
    maxReg = 0
    ret_ = 0
    
    for command in commands:
        reg_ = commands[command][0]
        inst_ = commands[command][1]
        val_ = int(commands[command][2])
        
        instr = commands[command][5]
        left = commands[command][4]
        right = int(commands[command][6])
        
        def calcReg(reg, inst, val):
            if inst == 'dec':
                register[reg] -= val
            elif inst == 'inc':
                register[reg] += val
        ...
        if instr == '>':
            if register[left] > right:
                ret_ = calcReg(reg_, inst_, val_)
                #print('higher')
        elif instr == '<':
            if register[left] < right:
                ret_ = calcReg(reg_, inst_, val_)
...
nr = 0
for line in data:
    register.setdefault(line.split()[0])
    register[line.split()[0]] = 0

    commands.setdefault(nr)
    commands[nr] = line.split()
    nr += 1
...
runReg(register, commands)
pp.pprint(register)

maximum = 0
for i in register:
    if register[i] > maximum:
        maximum = register[i]
